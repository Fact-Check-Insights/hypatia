...
module Zorki
  class User
    ...
    # Build a User from an item["user"] XDTUserDict payload returned by the
    # xdt_api__v1__media__shortcode__web_info endpoint. Follower / post counts
    # are not present in that payload, so they remain nil.
    def self.from_items_user(data)
      return nil if data.nil?

      profile_image_url = data.dig("hd_profile_pic_url_info", "url") || data["profile_pic_url"]
      profile_image = profile_image_url ? Zorki.retrieve_media(profile_image_url) : nil
    ...
    # Build a minimal User from a schema.org Person payload (the articleBody
    # branch). Counts and bio are not reliably present here.
    def self.from_schema_org_author(author, graphql_object = {})
      return nil if author.nil?

      username = author.dig("identifier", "value")
      profile_image_url = author["image"]
      profile_image = profile_image_url ? Zorki.retrieve_media(profile_image_url) : nil
      ...
      User.new(
        ...
        profile: graphql_object["description"],
        profile_link: author["sameAs"] || (username && "https://www.instagram.com/#{username}"),
        profile_image: profile_image,
        profile_image_url: profile_image_url
      )
    end

    # Build a minimal User from just a username, used as a last-resort fallback
    # when no richer payload is available.
