...
module Zorki
  class UserScraper < Scraper
    def parse(username)
      ...
      graphql_script = nil
      # Bounded retry budget: 3 attempts with progressively longer polling
      # timeouts per inner-query variant. Total worst-case (3 attempts × 3
      # variants) ≈ 30 + 60 + 90 = 180s, vs. the previous 12-minute budget.
      attempt_timeouts = [10, 20, 30]
      attempt_timeouts.each_with_index do |per_query_timeout, idx|
        login
        print "Scraping user #{username}... (attempt #{idx + 1}/#{attempt_timeouts.length}, per-query timeout #{per_query_timeout}s)\n"
        begin
          ...
          begin
            graphql_script = get_content_of_subpage_from_url("https://instagram.com/#{username}/", "graphql/query", "data,user,follower_count", timeout: per_query_timeout)
          rescue Zorki::ContentUnavailableError
            begin
              graphql_script = get_content_of_subpage_from_url("https://instagram.com/#{username}/", "graphql/query", "data,user,friendship_count", timeout: per_query_timeout)
            rescue Zorki::ContentUnavailableError
              graphql_script = get_content_of_subpage_from_url("https://instagram.com/#{username}/", "graphql/query", nil, post_data_include: "render_surface%22%3A%22PROFILE", header: { "X-FB-Friendly-Name": "PolarisProfilePageContentQuery" }, timeout: per_query_timeout)
            end
          end
          ...
          if idx == attempt_timeouts.length - 1
