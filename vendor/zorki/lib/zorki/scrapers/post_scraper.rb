  ...
  class PostScraper < Scraper
    ...
    def attempt_parse(id)
      ...
      if graphql_object.has_key?("articleBody")
        ...
        # Post-payload-derived User, only used as fallback if the live User.lookup
        # below fails. The original flow still tries User.lookup first.
        fallback_user = begin
          User.from_schema_org_author(graphql_object["author"], graphql_object)
        rescue StandardError
          nil
        end
        ...
        # below fails. The original flow still tries User.lookup first.
        fallback_user = begin
          User.from_items_user(item["user"])
        rescue StandardError
          nil
        end
      ...
      # Preserve the original flow: still try the live profile scrape so we get
      # follower / post counts when Instagram serves them. Defensive catch falls
      # back to whatever the post payload already gave us (or, as a last resort,
      # a username-only stub) instead of letting a user-profile failure kill the
      # whole post scrape.
      if defined?(user) && user.nil?
        begin
          user = User.lookup([username]).first
        rescue Zorki::UserScrapingError => e
          puts "User lookup for #{username} failed: #{e.message}"
          user = (defined?(fallback_user) && fallback_user) || User.from_username(username)
          puts "Falling back to post-payload User: #{user&.username.inspect}"
        rescue StandardError => e
          puts "Unexpected error during User.lookup for #{username}: #{e.class}: #{e.message}"
          user = (defined?(fallback_user) && fallback_user) || User.from_username(username)
        end
      end
      {
