...
module Zorki
  class Scraper # rubocop:disable Metrics/ClassLength
    ...
    def get_content_of_subpage_from_url(url, subpage_search, additional_search_parameters = nil, post_data_include: nil, header: nil, timeout: 60)
      ...
      # Poll for the matching intercepted response (up to `timeout` seconds)
      start_time = Time.now
      while response_body.nil? && (Time.now - start_time) < timeout
        begin
